# -*- coding: utf-8 -*-
"""Copy of Atividade_avaliativa_SQL.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1L1T3uLU4OQonJ7I-NLAnXBmf-bbqLHK5
...
SELECT P.Sigla,
       P.NomePro
FROM Projeto as P, Cliente as C
WHERE P.CodCli = C.CodCli AND C.NomeCli <> 'Facebook'
"""
...
WHERE L.NomeLoc IN (
  SELECT V.Destino
  FROM Viagem AS V JOIN Consultor  AS C
  ON V.CPF = C.CPF
...
SELECT P.Sigla,
       P.NomePro
FROM Projeto as P LEFT JOIN Equipe as E
ON P.Sigla = E.Sigla
...
query = """
SELECT P.Sigla, P.NomePro
FROM Projeto AS P
WHERE P.Sigla  NOT IN(
  SELECT E.Sigla
  FROM Equipe AS E JOIN Consultor AS C
  ON E.CPF = C.CPF
...
FROM Consultor AS C
WHERE C.UFNascCon IN (
  SELECT L.UFLoc
  FROM Local AS L, Viagem AS V
  WHERE L.NomeLoc = V.Origem AND V.CPF = C.CPF
)
OR
C.UFNascCon IN (
  SELECT L.UFLoc
  FROM Local AS L, Viagem AS V
  WHERE L.NomeLoc = V.Destino AND V.CPF = C.CPF
  )
"""
...
query = """
SELECT L.NomeLoc
FROM Local as L LEFT JOIN Viagem as V
...
WHERE P.Sigla IN (
  SELECT V.Sigla
  FROM Viagem AS V
  WHERE strftime('%Y', V.DataViagem) >= '2019' AND V.Destino IN(
    SELECT L.NomeLoc
    FROM Local AS L
    WHERE L.UFLoc = 'CA'
  )
...
FROM Viagem AS V JOIN Consultor AS C 
ON v.CPF = C.CPF
WHERE (V.CPF,V.Sigla) NOT IN(
  SELECT E.CPF, E.Sigla
  FROM Equipe AS E JOIN Consultor AS C
  ON E.CPF = C.CPF
)
"""
...
WHERE C.CPF NOT IN(
  SELECT E.CPF
  FROM Equipe As E JOIN Projeto AS P
  ON E.Sigla = P.Sigla
  WHERE P.DataInicio >= 2019
)
"""
